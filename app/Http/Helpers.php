...
if (! function_exists('home_discounted_price')) {
    function home_discounted_price($id,$qte = null,$coupon = null,$shipping = null)
    {
        $product = Product::findOrFail($id);
        $lowest_price = $product->unit_price;
        $highest_price = $product->unit_price;
        ...
        if($coupon !== null){
            if(\Session::has('coupon_discount')){
                $lowest_price -= \Session::get('coupon_discount');
                $highest_price -= \Session::get('coupon_discount');
            }
        }


        if($qte !== null){
            $lowest_price = $lowest_price * floatval($qte);
            $highest_price = $highest_price * floatval($qte);
        }
        if($shipping !== null){
            $lowest_price = $lowest_price + $shipping;
            $highest_price = $highest_price + $shipping;
        }
...
if (! function_exists('home_discounted_price_num')) {
    function home_discounted_price_num($id,$qte = null,$coupon = null,$shipping = null, $cartItem = null)
    {
        $product = Product::findOrFail($id);
        // $lowest_price = $product->unit_price;
        // $highest_price = $product->unit_price;
        $lowest_price = isset($cartItem['price']) ? $cartItem['price'] : $product->unit_price;
        $highest_price = isset($cartItem['price']) ? $cartItem['price'] : $product->unit_price;
        if ($product->variant_product) {
            foreach ($product->stocks as $key => $stock) {
                if($lowest_price > $stock->price){
                    $lowest_price = $stock->price;
...
if (! function_exists('home_discounted_price_ttc')) {
    function home_discounted_price_ttc($id,$tva)
    {
        $product = Product::findOrFail($id);
        $lowest_price = $product->unit_price;
        $highest_price = $product->unit_price;
        ...
        $lowest_price = convert_price($lowest_price + $lowest_price*25/100);
        $highest_price = convert_price($highest_price + $highest_price*25/100);
