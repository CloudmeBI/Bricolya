...
//            $product->slug = preg_replace('/[^A-Za-z0-9\-]/', '', str_replace(' ', '-', $product->name)).'-'.str_random(5);
//            $product->save();
//        }
...
Route::post('/users/registration', 'HomeController@createAccount')->name('user.registration');
...
//Export all product in pdf
Route::get('/export_all_products_pdf', 'HomeController@exportAllProducts')->name('pdf.export_product_pdf');
########## contact us page ###########################
...
########### Product page ##############################
Route::get('/all_products', 'HomeController@all_products')->name('all_products');
########## Flash product page #########################
Route::get('/flash_products', 'HomeController@flash_products');
########## About us page ###############################
Route::get('/aboutus', 'HomeController@aboutus');
########## supplier login page ##########################
Route::post('/supplier/login','HomeController@loginSupplier')->name('supplier.login');
Route::get('/supplier/login','HomeController@supplierLogin')->name('supplier.login.get');
Route::get('/supplier/register','HomeController@supplierRegisterGet')->name('supplier.register.get');
Route::post('/supplier/register','HomeController@supplierRegister')->name('supplier.register');
########## reseller login page ##########################
Route::get('/reseller/login','HomeController@resellerLogin')->name('reseller.login.get');
Route::post('/reseller/login','HomeController@reseller_login');
Route::get('/reseller/register','HomeController@reseller_register_get')->name('reseller.register.get');
Route::post('/reseller/register','HomeController@reseller_register')->name('reseller.register');
############ send contct us email #######################
Route::post('/send_contact_mail','HomeController@send_contact_mail');
############  recruitment mail ############################
Route::post('/send_recruitment','HomeController@send_recruitment_email');
...
Route::get('/product/{slug}/{flash_deal_id?}', 'HomeController@product')->name('product');
...
Route::group(['middleware' => ['auth']], function(){
	...
	Route::get('/video_page', 'VideoController@index');
	Route::get('/video_page/create', 'VideoController@create');
	Route::post('/video_page/store', 'VideoController@store');
    ...
    Route::get('/catalogue_client/get', 'HomeController@catalogueClient')->name('catalogue.client.get');
    Route::post('/catalogue_client/store', 'HomeController@catalogueClientStore')->name('catalogue.client.store');
});
...
//Route::match(['get','post'],'/botman','BotManController@tinker');
